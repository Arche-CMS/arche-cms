...
export function createCollectionRouter(
  ...
  if (collection.versions?.drafts) {
    routes.push(
      ...
      {
        handler: applyMiddleware(createBulkPublishHandler(collection, adapter), hooks, collection),
        method: "POST",
        operationId: `bulkPublish${pascalCase(slug)}`,
        path: `${basePath}/${slug}/bulk-publish`,
        summary: `Bulk publish ${plural}`,
        tags: [tag],
      },
      {
        handler: applyMiddleware(
          createBulkUnpublishHandler(collection, adapter),
          hooks,
          collection,
        ),
        method: "POST",
        operationId: `bulkUnpublish${pascalCase(slug)}`,
        path: `${basePath}/${slug}/bulk-unpublish`,
        summary: `Bulk unpublish ${plural}`,
        tags: [tag],
      },
    );
  }
